...
function handleSliderModeChange (checked: boolean) {
  checked
    ? (data.value.options.defaultValue = [10, 90])
    : (data.value.options.defaultValue = 0)
}

function handleSelectModeChange (val: boolean) {
  if (data.value.type === 'img-upload') return

  if (val) {
    if (data.value.options.defaultValue) {
      if (!Array.isArray(data.value.options.defaultValue)) {
...
<template>
  ...
  >
    ...
    >
      <el-input-number
        v-model.number="data.options.step"
        :min="0"
      />
    </el-form-item>
